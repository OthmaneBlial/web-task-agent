...
- Publish a GitHub release only after choosing its version and release notes. Publish to npm only after confirming registry ownership and the exact tarball. The manual GitHub Packages workflow publishes the scoped mirror `@othmaneblial/web-task-agent` with its ephemeral Actions token; it does not publish to npmjs.com.
