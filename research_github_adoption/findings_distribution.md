...
- [GitHub Marketplace](https://docs.github.com/en/actions/how-tos/create-and-publish-actions/publish-in-github-marketplace) can distribute a public Action, but Marketplace expects one root `action.yml` and a dedicated action repository is the cleanest packaging model.
...
4. Ship a read-only GitHub Action that validates receipts and explains diffs in pull requests.
...
The defensible loop is artifact-driven: an agent produces research, the user turns it into a receipt, CI verifies or diffs it, reviewers challenge specific evidence, those challenges become fixtures or adapters, and the receipt/action link exposes the project to the next repository.
