...
function zipBase64(files) {
  // ZIP stores a wall-clock DOS timestamp without a timezone. Constructing the
  // epoch in local time keeps the generated bytes identical on CI and macOS.
  const zipEpoch = new Date(1980, 0, 1, 0, 0, 0);
  const archive = Object.fromEntries(Object.entries(files).map(([filePath, content]) => [
    filePath,
    [strToU8(content), { mtime: zipEpoch }]
  ]));
  return Buffer.from(zipSync(archive, { level: 6, mtime: zipEpoch })).toString("base64");
}
